...
for i, dept_name in enumerate(dept_names):
    if cols[i].button(dept_name):
        ...
        play_female_tts(data[dept_name]["voice"])
...
with col_left:
    # 💡 선생님의 'image.jpg' 파일명과 정확히 일치시켰습니다.
    local_img = "image.jpg"
    if os.path.exists(local_img):
        st.image(local_img, use_container_width=True)
    else:
        st.info(f"💡 '{local_img}' 파일을 main.py와 같은 폴더에 올려주시면 세 자매가 나타납니다!")
